...
$\BA_n^{s_1, \dotsc, s_n}$. It~is parameterized by~the \emph{significance vector}
...
\end{theorem}
\begin{proof}
    To~compute $\BA_n^s$, apply the following method (mentioned in~the introduction).
    \begin{quote}
        Process the bits layer by~layer, in~the order of~increasing significance.
        While the current significance layer~$i$ contains at~least three bits,
        take three of~them and apply~FA to~replace them with a~pair of~bits
        of~significance $i$~and~$i+1$. If~there are two bits left at~the current layer~$i$, apply~HA to~them to~get a~pair of~bits of~significance $i$~and~$i+1$.
    \end{quote}
    Each application of a~Full Adder reduces the number of~bits by~one,
    ...
    the total cost of~all Half Adders is~at~most~$2m$.
\end{proof}

Note that, for many vectors~$s$, the upper bound
is~$\operatorname{size}(\BA_n^s) \le 5n-3m$ is~loose:
...
is~\emph{provably} optimal. For example, for the $\ADD_n$ function,
the method builds a~circuit out of a~single Half Adder and $(n-1)$
Full Adders. The resulting circuit has size $5n-3$
and Red'kin~\cite{Red81} proved that there~is no~smaller circuit
for this function.
